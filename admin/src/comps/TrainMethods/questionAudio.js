...
//
// Раньше вопрос всегда читал робот браузера (speechSynthesis). Теперь бэкенд
// заранее генерирует WAV голосом-образцом через адаптер audio.tts_generate.v1,
// кладёт файл в закрытый бакет S3 и по запросу отдаёт короткоживущий presigned
// URL: POST /api/question-audio/url {text}.
//
// Робот остаётся запасным вариантом, а не удаляется: аудио может быть ещё не
// сгенерировано (новый или только что отредактированный вопрос), озвучка может
// быть выключена на сервере, а ссылка - не проиграться в браузере.

let currentAudio = null;

// Останавливает проигрывание готового файла - зовётся из stopAnyPlay вместе с
// speechSynthesis.cancel(), чтобы предыдущий вопрос не звучал поверх нового.
...
// для него это не сбой, вопрос всё равно будет прочитан.
...
// и вопрос надо прочитать роботом.
